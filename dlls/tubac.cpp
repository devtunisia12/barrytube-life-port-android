...
LINK_ENTITY_TO_CLASS(monster_tubac, CHAssault);
...
void CHAssault::Spawn()
{
	...
	SET_MODEL(ENT(pev), "models/tubac.mdl");
...
{
	PRECACHE_MODEL("models/tubac.mdl");
...
void CHAssault::Killed(entvars_t* pevAttacker, int iGib)
{
    ...
    Vector vecGunAngles;
	SpinDown();
	DropItem( "weapon_chaingun", vecGunPos, vecGunAngles );
